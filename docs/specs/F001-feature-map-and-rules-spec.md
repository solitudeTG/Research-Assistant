...
- 上传论文或研究文档。
- 以 GROBID 为论文 PDF 主解析栈，Docling/PyMuPDF 为 fallback，解析结果进入 Research Assistant 自有 canonical paper model。
- 解析并切分文档，保留 source identity、section、page、chunk_id 等 citation 所需边界。
- 将 chunks 索引到 PostgreSQL + pgvector，并同步建立 PostgreSQL full-text search 索引。
- 提出有依据的问题。
- 使用 PostgreSQL full-text search + pgvector 的混合检索召回证据。
- 生成带 citation 的答案。
- 将过程事件流式推送到 UI。
- 在工作台中展示 Markdown 研究产物和证据产物。
...
- 最终答案包含可追溯 citation。
- 过程 trace 反映真实后端事件。
- P0 只要求 uploaded papers 作为 evidence source；web search 延后到后续版本。
...
### Research Data Store Boundary

Research Assistant 的研究领域主存储是 PostgreSQL + pgvector。

必须进入 PostgreSQL 研究领域存储的数据包括：

- papers / paper files metadata。
- canonical paper model records。
- chunks、embeddings 和 full-text index。
- evidence records、citation records 和 audit results。
- report evidence map。
- 与 citation/context 边界相关的 memory records。

MongoDB 暂保留给 ScienceClaw baseline application shell 的既有 operational data，例如用户、会话、配置和任务等。除非另有 ADR，新增 Research Assistant 核心领域数据不得默认写入 MongoDB。
...
- 文档解析：GROBID primary，Docling/PyMuPDF fallback。
- 检索：P0 直接做 PostgreSQL full-text search + pgvector hybrid retrieval，使用 RRF 或简单加权融合；reranker 后续增强。
- 报告：P0 只要求 Markdown，DOCX/PDF 作为后续 exporter。
- Web search：P0 延后，先聚焦 uploaded papers。
- 存储：PostgreSQL + pgvector 是 Research Assistant 研究领域主存储；MongoDB 仅保留 ScienceClaw baseline 壳既有 operational data。
- 决策来源：[ADR-001: P0 Research Data and Retrieval Stack](../decisions/ADR-001-p0-research-data-and-retrieval-stack.md)。
