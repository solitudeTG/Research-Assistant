...
- The configured model call failed with `PermissionDeniedError`; the answer safely downgraded to deterministic audit and recorded `semantic_auditor.mode=llm_failed`.
- Review result: this fallback artifact is not accepted as F023 live Case C completion evidence. Case C now requires `semantic_auditor.mode=llm_enhanced`, claim-level `llm_support_status`, `llm_rationale`, and an `llm_*` finding code.
...
Partial pass; live LLM acceptance blocked.
...
- Live model call returned `PermissionDeniedError`, so the live artifact proves safe degradation only. It does not prove live `llm_enhanced` auditor judgment.
...
- The live UI artifact does not prove a successful live `llm_enhanced` auditor call because the configured provider rejected the request with `PermissionDeniedError`.
- F023 remains active until a live UI Case C artifact proves `semantic_auditor.mode=llm_enhanced`.
