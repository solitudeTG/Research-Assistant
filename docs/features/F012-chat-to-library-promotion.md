...
| F012.2 | 2026-06-29 | `2ce52be` | Unbound Chat sessions could infer a promotion candidate from uploaded-paper metadata and show `加入研究库`, even though there was no selected Research Project boundary. | `ChatMessage` reconstructed promotion candidates from message metadata without knowing whether the parent Chat session had a linked Project. | Added a frontend contract requiring `canPromoteToResearchLibrary`, passed from `ChatPage` as `!!currentResearchProject`; focused contract, full frontend contract suite, type-check, and build passed. | verified |
