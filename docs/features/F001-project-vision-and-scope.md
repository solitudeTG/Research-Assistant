...
| Memory records are bounded as context-only research data. | PostgreSQL research storage includes `research_memory_entries` for L1/L2/L3 memory records with database constraints forcing `source_type='memory'` and `context_only=true`; repository and database wrappers persist and list memory entries as context dictionaries that cannot be treated as citation evidence. This is the storage boundary for future recall/promotion work, not full three-layer memory productization. | Red tests first failed for missing memory table and storage APIs; green verification passed with `python -m pytest ScienceClaw\backend\tests\test_research_store_schema.py ScienceClaw\backend\tests\test_research_repository.py ScienceClaw\backend\tests\test_research_database.py --basetemp .pytest_tmp\run -q` -> `20 passed`, full `python -m pytest ScienceClaw\backend\tests -k research --basetemp .pytest_tmp\run -q` -> `47 passed`, and `python -m py_compile ScienceClaw\backend\research_assistant\storage\repository.py ScienceClaw\backend\research_assistant\storage\database.py` passed on 2026-06-21. | Partial |
