...
- Known gaps: P0 uploaded-paper research loop is verified and now has a durable checked-in browser E2E runner. The first Evidence Audit slice is deterministic and claim-to-citation based; audit results and individual citation evidence records are now durable and recoverable through PostgreSQL-backed API paths, Chat can inspect individual citation records, Chat can recover persisted audit panels for answer/report subjects, Markdown reports now expose claim-level audit checks, audit-gated findings, approved-claim evidence maps, and claim support scores, and memory records now have a PostgreSQL context-only storage boundary, separate answer/UI channel, explicit approved-claim promotion path, duplicate-safe promotion behavior for the same subject claim, session-scoped revocation, deterministic recall scoring, and richer recall reasons. Richer audit heuristics beyond lexical support, multi-source audit, broader composer gating, polished report composition, advanced memory scoring thresholds/decay, and cross-session deduplication remain follow-up work. Markdown reports are still extractive evidence notes rather than polished composed reports.
