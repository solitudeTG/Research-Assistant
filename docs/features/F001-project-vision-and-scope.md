...
| Markdown research artifacts can be generated from uploaded paper evidence. | A research report route writes Markdown and an evidence map into the session workspace, persists report-to-evidence rows in PostgreSQL, emits real step/done events, exposes files through existing round file/file panel mechanics, includes per-claim Evidence Audit checks in the Markdown artifact itself, gates the report findings section to claims approved by Evidence Audit, and maps sidecar/database evidence rows to the approved report claim text. | `ScienceClaw\backend\tests\test_research_reports.py`, repository/database tests, `py_compile`, and frontend build passed on 2026-06-19. Claim-check report follow-up: focused report test, full research test suite, `py_compile`, frontend `type-check`, and frontend build passed on 2026-06-21. Report-gating follow-up: focused report test and full research test suite passed on 2026-06-21. Evidence-map alignment follow-up: focused report test, full research test suite, `py_compile`, frontend `type-check`, frontend build, and Harness strict passed on 2026-06-21. | Partial |
...
- Known gaps: P0 uploaded-paper research loop is verified and now has a durable checked-in browser E2E runner. The first Evidence Audit slice is deterministic and claim-to-citation based; audit results and individual citation evidence records are now durable and recoverable through PostgreSQL-backed API paths, paper/web/database citation source types share the same citation-evidence contract, Chat can inspect individual citation records, Chat can recover persisted audit panels for answer/report subjects, Markdown reports now expose claim-level audit checks, audit-gated findings, approved-claim evidence maps, and claim support scores, and memory records now have a PostgreSQL context-only storage boundary, separate answer/UI channel, explicit approved-claim promotion path, duplicate-safe promotion behavior for the same subject claim, session-scoped revocation, deterministic recall scoring, and richer recall reasons. Actual web/database ingestion, richer audit heuristics beyond lexical support, multi-source audit workflows, broader composer gating, polished report composition, advanced memory scoring thresholds/decay, and cross-session deduplication remain follow-up work. Markdown reports are still extractive evidence notes rather than polished composed reports.
- Next safe action: Move to advanced memory lifecycle/cross-session deduplication, richer Evidence Audit, report quality, or the next scoped research workflow.
